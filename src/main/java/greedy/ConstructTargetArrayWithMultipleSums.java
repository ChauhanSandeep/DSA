...
public class ConstructTargetArrayWithMultipleSums {
    ...
    private boolean canConstruct(int[] arr, long sum) {
        ...
        long previousValue = maxElement % remainingSum;// modulo is used instead of - because there can be multiple additions
     ...
     * 3. For largest element maxValue, calculate what it was before: maxValue % (totalSum - maxValue)
     ...
     */
    public boolean isPossibleIterative(int[] target) {
        ...
        while (maxHeap.peek() > 1) {
            ...
            // If remaining sum is 1, we can always form the array.
            // Eg [maxValue, 1, 1, 1, ...] here in the end we will have [1, 1, 1, 1 ...]
            if (remainingSum == 1) return true;

            // If the remaining sum is greater than or equal to the max value or remaining sum is 0, it's not possible to form the array
            if (remainingSum >= maxValue || remainingSum == 0) return false;

            // Optimization: instead of repeatedly subtracting, use modulo
            long previousValue = maxValue % remainingSum;

            // Edge case Handle: Where maxValue is exactly (k * remainingSum)
            if (previousValue == 0) {
                // Check if maxValue was created by adding remainingSum to 1
                if (maxValue == remainingSum + 1) {
                    previousValue = 1;
                } else {
                    return false;
                }
