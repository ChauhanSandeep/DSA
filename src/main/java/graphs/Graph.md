...
1. **Adjacency List**: A list of lists, where each list describes the set of neighbors of a vertex.
...
1. **DFS-based Topological Sort**: Uses DFS with a **recursion stack** (or three-state marking) so a back edge detects a **cycle**; finishes by pushing vertices in post-order, then popping for the order. Time: O(V + E).
2. **Kahn's Algorithm (Preferred)**: Uses BFS with in-degree tracking. Repeatedly removes vertices with zero in-degree and reduces in-degree of neighbors. Easier to reason about; detects cycles when fewer than \|V\| nodes are processed. Time: O(V + E).
...
1. Keep `onStack[v]` for the current DFS path and `finished[v]` for vertices already pushed to the post-order stack.
2. At `v`: if `onStack[v]`, **cycle**; if `finished[v]`, return.
3. Set `onStack[v] = true`, recurse on neighbors; on any failure, abort.
4. Clear `onStack[v]`, set `finished[v] = true`, push `v` onto the stack.
5. If any component reports a cycle, return an **empty list** (and discard partial stack contents). Otherwise pop the stack to get the topological order.
...
/**
 * Topological sort of a directed graph using DFS. Detects cycles via the recursion stack.
 *
 * @return Topological order, or empty list if a cycle exists (same contract as Kahn’s version in this doc).
 *
...
public class TopologicalSortDFS {
  ...
  public static List<Integer> topologicalSort(List<List<Integer>> adjacencyList) {
    ...
    boolean[] finished = new boolean[totalVertices]; // true -> the node is processed successfully
    boolean[] onStack = new boolean[totalVertices]; // true -> This node is in current path
    Deque<Integer> dfsStack = new ArrayDeque<>();

    for (int vertex = 0; vertex < totalVertices; vertex++) {
      if (!finished[vertex]) {
        if (!dfs(vertex, adjacencyList, finished, onStack, dfsStack)) {
          dfsStack.clear();
  ...
  /**
   * @return false if a back edge (cycle) is found; true if this subtree is acyclic and pushed in post-order.
   */
  private static boolean dfs(int currentVertex, List<List<Integer>> graph,
      boolean[] finished, boolean[] onStack, Deque<Integer> dfsStack) {
    ...
    for (int neighbor : graph.get(currentVertex)) {
      if (!dfs(neighbor, graph, finished, onStack, dfsStack)) {
        return false;
      }
    }
...
public class BellmanFord {
     ...
     */
    public static int[] findShortestPaths(int source, int numVertices, List<Edge> edges) {
        int[] distance = new int[numVertices];
        Arrays.fill(distance, Integer.MAX_VALUE);
...
++**This is not important for interviews**++
...
public class PrimMST {
  ...
  public static int computeMST(int numVertices, List<List<Edge>> graph) {
    ...
    while (!minHeap.isEmpty() && edgesAdded < numVertices) {
      ...
      // MARK(*): Skip if already marked, else mark now
