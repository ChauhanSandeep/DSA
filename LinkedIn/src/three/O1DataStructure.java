 ...
 * - Reassign pointers to transform the tree.
 *
...
public class TreeFlipTest {
    ...
    public static TreeNode flipUpsideDown(TreeNode root) {
        if (root == null || root.left == null) {
            return root; // Base case: return if tree is empty or single-node
        }

        // Recursively reach the leftmost node
        TreeNode newRoot = flipUpsideDown(root.left);

        // Rearranging pointers to flip the tree
        root.left.left = root.right;  // The right child becomes the new left child
        root.left.right = root;       // The parent becomes the new right child
        root.left = null;             // Remove original references
        root.right = null;

        return newRoot; // Return the new root (leftmost node)
    }
