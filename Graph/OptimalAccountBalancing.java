...
public class OptimalAccountBalancing {
    ...
    private int getMinTransfersAfter(int currId, int[] debt) {
        while (currId < debt.length && debt[currId] == 0) {
            currId++;
        }

        if (currId == debt.length) return 0;

        int minTransactions = Integer.MAX_VALUE;
        for (int i = currId + 1; i < debt.length; i++) {
            if (debt[i] * debt[currId] < 0) {
                debt[i] += debt[currId];
                minTransactions = Math.min(minTransactions, getMinTransfersAfter(currId + 1, debt) + 1);
                debt[i] -= debt[currId];
            }
        }
