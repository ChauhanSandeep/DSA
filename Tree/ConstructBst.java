...
public class ConstructBst {
     ...
     * @param preorder   The preorder traversal array.
     * @param inorder    The inorder traversal array.
     * @param inLeft     The left index of the inorder array for the current subtree.
     * @param inRight    The right index of the inorder array for the current subtree.
     * @param preStart   The current index in the preorder array.
     * @return           The root of the current subtree.
     */
    private TreeNode buildTree(int[] preorder, int[] inorder, int inLeft, int inRight, int preStart) {
        // Base case: if the current segment is invalid, return null
        if (inLeft > inRight || preStart >= preorder.length) {
            return null;
        }
        ...
        int rootIndexInInorder = findIndex(inorder, preorder[preStart]);
        
        // Recursively build the left and right subtrees
        // Left subtree will be constructed using elements before the root in inorder
        TreeNode leftSubtree = buildTree(preorder, inorder, inLeft, rootIndexInInorder - 1, preStart + 1);
        
        // Right subtree will be constructed using elements after the root in inorder
        TreeNode rightSubtree = buildTree(preorder, inorder, rootIndexInInorder + 1, inRight, preStart + 1 + (rootIndexInInorder - inLeft));
